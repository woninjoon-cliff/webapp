...
function HOSPITAL_현황(token) {
  ...
  // 매출 집계 (2026-08-21 연결).
  // 05_시술의 시술금액을 합산한다. 취소 건은 제외하며,
  // 집계 규칙은 PROC_getSummary 와 동일하게 맞춘다 (같은 화면에서 숫자가 갈리면 안 된다).
  const 오늘 = PROC_날짜문자열_(new Date());
  const 당월 = 오늘.substring(0, 7);

  let 금일매출 = 0;
  let 월매출 = 0;

  행가져오기('시술').forEach(function(row) {

    if (String(row['상태']) === PROC_상태_취소) {
      return;
    }

    const 일자 = PROC_날짜문자열_(row['시술일']);
    const 금액 = Number(row['시술금액']) || 0;
