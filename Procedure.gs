...
//
// - 대상 시트: 05_시술 (19열, 용량/단위/시술금액/패키지여부/분류 포함)
...
function PROC_시트재구성() {
  ...
  if (기존헤더.join('|') === PROC_새헤더.join('|')) {
    Logger.log('05_시술 이미 새 구조(19열). 변경 없음.');
    return;
  }
...
function test_PROC_전체흐름() {
  ...
  try {
    ...
    Logger.log('5. 재조회 반영 확인(분류 갱신 포함): ' + (수정확인 === 1 ? '정상' : '실패'));
